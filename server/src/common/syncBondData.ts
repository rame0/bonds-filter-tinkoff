...
function normalizeBondInstrument(bond: ApiBond, updatedAt: string): BondInstrumentRecord {
	...
	return {
		...
		collateralFlag: Boolean(normalized.collateralFlag),
		taxFreeFlag: Boolean(normalized.taxFreeFlag),
		shortEnabledFlag: Boolean(normalized.shortEnabledFlag),
		...
		blockedTcaFlag: Boolean(normalized.blockedTcaFlag),
